...
//Sleep mode added by Sascha Rutz
...
volatile uint32_t sleepcount = 0;	//Variable used to Check if sleep should be enabled
...
int main(void)
{
	...
	{ 	
		...
		if(GRUEN_MINUS || GRUEN_PLUS || BLAU_MINUS || BLAU_PLUS || ROT_MINUS || ROT_PLUS)
		{
			sleepcount = 0;	//Reset time since last Input if there is a Input
		}
		
		sleepcount++;
			if (sleepcount >= 2000000)	//Check if it has been long enough since last input
			{
				//Turn off LEDs
				//======================================================
				NROT;
				...
				//Enter sleep mode
				//======================================================
				set_sleep_mode(0b010);	//Set sleep mode to power-down
				cli();	//disable interrups
				sleep_enable();	//Enable sleep
				sleep_bod_disable();	//Disable Brown Out Detector
				sei();	//Enable Interups
				sleep_cpu();	//Set cpu to sleep
				//======================================================
			}
...
ISR (INT1_vect)
{
	if(sleepcount >= 2000000)	//Check if in sleep mode
	{
		//Wake up
		//==============================================================
	...
	sleepcount = 0;	//Reset time since last input
}
